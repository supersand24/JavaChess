        ...
        if (xTarget == xPos) {
                ...
                if (moved) {
                    ...
                    if ((yTarget - yPos) > 0) {
                        if (checkSpace(xPos,yPos + 1,board)) {
                            System.out.println("Path Blocked, there is a " + getArtifactName(xTarget, yTarget, board) + " in the way, at " + convertCoordToChessPlace(xPos,yPos + 1,board));
                        } else {
                            canMove = true;
                        }
                    //Going Up
            ...
            //If target is one space away and on same column
                ...
                if (checkSpace(xTarget,yTarget,board)) {
                    System.out.println("Invalid Move, there is a " + getArtifactName(xTarget, yTarget, board) + " at " + convertCoordToChessPlace(xTarget,yTarget,board) + ", and a Pawn may not capture whats directly in front of it.");
                } else {
                    canMove = true;
                }
            } else {
